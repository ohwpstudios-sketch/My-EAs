...
void ApplyATRChandelierTrail()
{
   for(int i=0;i<PositionsTotal();i++)
   {
       ...
       {
          double newSL = g_tick.bid - trail;
          if(newSL > sl && newSL < g_tick.bid) Trade.PositionModify(_Symbol, newSL, tp);
       }
       else if(type==POSITION_TYPE_SELL)
       {
          double newSL = g_tick.ask + trail;
          if(newSL < sl && newSL > g_tick.ask) Trade.PositionModify(_Symbol, newSL, tp);
       }
   }
}
