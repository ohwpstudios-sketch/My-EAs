//+------------------------------------------------------------------+
//|                  TrendGuard Pro v8 - PropMaster MSM              |
//|        Single-chart Multi-Symbol Manager + Prop Guardrails       |
//|        EURUSD, GBPUSD, XAUUSD, NAS100, BTCUSD (configurable)     |
//|        2025-09 - Final Reconstructed Clean Build (MT5)           |
//+------------------------------------------------------------------+
...
input int     CooldownMinutesAfterLoss          = 30;         // pause for N minutes after loss
...
bool SessionOkayAndSizeMult(double &mult_out){
   ...
   if(!(tok||lon||ny)){
      if(VerboseLog) Print("TG8: Outside sessions");
      return false;
   }
   ...
   if(DebugMode)
      Print("TG8 Debug Session: tok="+IntegerToString((int)tok)+" lon="+IntegerToString((int)lon)+
            " ny="+IntegerToString((int)ny)+" mult="+DoubleToString(mult_out,2));
   return true;
}
void UpdateEquityCurve(){
   ...
   double sum=0.0; for(int i=0;i<N;i++) sum+=g_equityHistory[i];
   g_equityMA = sum/N;
}
bool CheckDailyAndTotalDD(){
   ...
   if(dayDD>=MaxDailyLoss_Percent){
      if(VerboseLog) Print("TG8: Daily loss breached. Pausing.");
   ...
   if(g_peakEquity>0){
      ...
      if(totalDD>=MaxTotalDrawdown_Percent){
         if(VerboseLog) Print("TG8: Total DD breached. Pausing.");
         return false;
      }
   }
   if(g_consecLosses>=MaxConsecutiveLosses){
      if(VerboseLog) Print("TG8: Max consecutive losses reached. Cooling down.");
      return false;
   }
   if(g_lastLossTime>0 && (TimeCurrent()-g_lastLossTime)<CooldownMinutesAfterLoss*60){
      if(VerboseLog) Print("TG8: Cooling down after loss.");
      return false;
   }
   return true;
}
...
//---------------- Portfolio helpers & execution ---------------------
bool WithinMaxPositionsSymbol(const string sym){
   int cnt=0;
...
double ActivePortfolioRiskPercentEstimate(){
   ...
   for(int i=0;i<PositionsTotal();i++){
      ...
      if(sl==0 || tickValue<=0 || tickSize<=0) continue;
...
bool CorrelatedBlock(const string sym,int dir){
   ...
   for(int i=0;i<PositionsTotal();i++){
      ...
      pdir=(pdir==POSITION_TYPE_BUY)? +1 : -1;
...
bool PlaceTrade(const string sym,int dir,double stop_pips,double rr_tp){
   if(!WithinMaxPositionsSymbol(sym)) return false;
   if(OpenPositionsTotal()>=MaxOpenPositionsTotal) return false;
   if(CorrelatedBlock(sym, dir)){ if(VerboseLog) Print("TG8: Correlation block on ",sym); return false; }
   ...
   if(g_equityMA>0 && AccountInfoDouble(ACCOUNT_EQUITY) < g_equityMA){
      effRisk*=0.5;
      if(VerboseLog) Print("TG8: Equity < MA, risk reduced for ",sym);
   }
   double activeRisk=ActivePortfolioRiskPercentEstimate();
   if(activeRisk + effRisk > MaxPortfolioRisk_Percent){
      if(VerboseLog) Print("TG8: Portfolio risk cap reached. Active=",DoubleToString(activeRisk,2),
                           "% + New=",DoubleToString(effRisk,2)," % > Max=",DoubleToString(MaxPortfolioRisk_Percent,2),"%");
      return false;
   }
   ...
   bool ok = (dir>0)? Trade.Buy(lots, sym, 0, sl, tp, "TG8-MSM")
                    : Trade.Sell(lots, sym, 0, sl, tp, "TG8-MSM");
   if(VerboseLog) Print("TG8: ",(dir>0?"BUY ":"SELL "),sym," risk=",DoubleToString(effRisk,2),
...
//=========================== LIFECYCLE ==============================
int OnInit(){
   int parts = StringSplit(SymbolsCSV, ',', g_symbols); // ',' as char literal
   if(parts<=0){ Print("TG8: No symbols in SymbolsCSV"); return(INIT_FAILED); }
   ArrayResize(g_lastBarTime, parts);
   for(int i=0;i<parts;i++){
      StringTrimLeft(g_symbols[i]);   // in-place
      StringTrimRight(g_symbols[i]);  // in-place
      g_lastBarTime[i]=0;
   }
   ResetDailyEquityAnchor();
   UpdateEquityCurve();
   EventSetTimer(PollSeconds);
   if(VerboseLog) Print("TG8 Init: TF=",EnumToString(WorkingTF)," Symbols=",SymbolsCSV);
...
void EvaluateSymbol(const string sym){
   ...
   if(idx<0) return;
   if(bar==g_lastBarTime[idx]) return;
...
void OnTick(){ /* timer-driven engine */ }

//------------------- Dashboard -------------------------------------
void DrawDashboard(){
   string name="TG8_MSM_Dashboard";
   ...
   string line="Equity: "+DoubleToString(eq,2)
              +" | DD Day: "+DoubleToString(dayDD,2)+"%/"+DoubleToString(MaxDailyLoss_Percent,1)+"%"
              +" | DD Total: "+DoubleToString(totalDD,2)+"%/"+DoubleToString(MaxTotalDrawdown_Percent,1)+"%"
              +" | RiskMode: "+((g_equityMA>0 && eq<g_equityMA)?"REDUCED 0.5x":"NORMAL");
   string hud="TrendGuard v8 MSM\n"+ line + "\n";
   for(int i=0;i<ArraySize(g_symbols);i++){
      ...
      for(int k=0;k<PositionsTotal();k++){
         if(!PositionSelect(k)) continue;
         if(PositionGetString(POSITION_SYMBOL)==s) posCount++;
      }
