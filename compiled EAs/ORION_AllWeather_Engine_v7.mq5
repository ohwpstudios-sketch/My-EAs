//+------------------------------------------------------------------+
//|                                     ORION_AllWeather_Engine_v7   |
//|                         Author: GH Wealth Makers Elite (c) 2025  |
//+------------------------------------------------------------------+
//| Multi‑regime engine (trend / range / shock) with:                |
//| - Vol targeting & equity‑MA entry gate                           |
//| - Partial TPs (2 layers) & ATR chandelier trailing               |
//| - Circuit breaker, sessions, spread/slippage guards              |
//| - **Smart movable dashboard** with drag handle + corner snapping |
//| - **Movable Mini HUD** (price‑follow or screen‑anchored)         |
//+------------------------------------------------------------------+
#property copyright   "2025 GH Wealth Makers Elite"
#property link        "https://ghwealthmakerselite.xyz/"
#property version     "1.07"     // Use two-part version to satisfy Market rules
#property strict
...
input int     InpPanelCorner           = CORNER_RIGHT_UPPER; // Initial corner
input int     InpPanelX                = 10;         // Initial X distance
input int     InpPanelY                = 20;         // Initial Y distance
input int     InpPanelWidth            = 360;        // Panel width
input int     InpPanelHeight           = 190;        // Panel height
input bool    InpPanelDraggable        = true;       // Make panel draggable
input bool    InpPanelFront            = false;      // Draw panel in front of candles
input bool    InpShowButtons           = true;       // Show control buttons

// Mini HUD (price-following or screen-anchored)
input bool    InpMiniHUD               = true;       // Show compact mini HUD
input bool    InpMiniHUDFollowPrice    = true;       // Follow price (OBJ_TEXT) else screen (OBJ_LABEL)
input int     InpMiniHUDOffsetPts      = 80;         // Vertical offset in points above price (follow mode)
input int     InpMiniHUD_FontSize      = 9;          // Font size
input color   InpMiniHUD_Color         = clrYellow;  // Text color
input bool    InpMiniHUD_UseComment    = false;      // Also write to Comment()
input int     InpMiniHUD_Corner        = CORNER_LEFT_UPPER; // Screen mode corner
input int     InpMiniHUD_X             = 20;                // Screen mode X
input int     InpMiniHUD_Y             = 40;                // Screen mode Y
...
string PANEL_HANDLE = "ORION_PANEL_HANDLE";
string HL_P1        = "ORION_HL_P1";
string HL_P2        = "ORION_HL_P2";
string MINI_HUD     = "ORION_MINI_HUD";

// Runtime UI positions (draggable state)
int  g_panelCorner, g_panelX, g_panelY;
int  g_miniCorner,  g_miniX,  g_miniY;
double g_miniOffsetPts;
...
void DrawPartialLines()
{
   ...
   if(!hasPos)
   {
      if(ObjectFind(cid, HL_P1) != -1) ObjectDelete(cid, HL_P1);
      if(ObjectFind(cid, HL_P2) != -1) ObjectDelete(cid, HL_P2);
      return;
   }

   if(InpUsePartialTP)
   {
      if(ObjectFind(cid, HL_P1) == -1) ObjectCreate(cid, HL_P1, OBJ_HLINE, 0, 0, p1);
   ...
   else if(ObjectFind(cid, HL_P1) != -1) ObjectDelete(cid, HL_P1);

   if(InpUsePartialTP2)
   {
      if(ObjectFind(cid, HL_P2) == -1) ObjectCreate(cid, HL_P2, OBJ_HLINE, 0, 0, p2);
   ...
   else if(ObjectFind(cid, HL_P2) != -1) ObjectDelete(cid, HL_P2);
}
...
//============================= MINI HUD ===============================//
void EnsureMiniHUDObject()
{
   if(!InpMiniHUD) return;
   long cid=ChartID();
   // Desired type per mode
   ENUM_OBJECT desiredType = (InpMiniHUDFollowPrice ? (ENUM_OBJECT)OBJ_TEXT : (ENUM_OBJECT)OBJ_LABEL);
   // If exists but type mismatch, recreate
   if(ObjectFind(cid, MINI_HUD) != -1)
   {
      int t = (int)ObjectGetInteger(cid, MINI_HUD, OBJPROP_TYPE);
      if(t != (int)desiredType) ObjectDelete(cid, MINI_HUD);
   }
   if(ObjectFind(cid, MINI_HUD) == -1)
   {
      ObjectCreate(cid, MINI_HUD, desiredType, 0, 0, 0);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_COLOR, InpMiniHUD_Color);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_SELECTABLE, true);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_SELECTED, false);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_HIDDEN, false);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_ZORDER, 3);
      ObjectSetString (cid, MINI_HUD, OBJPROP_FONT, "Consolas");
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_FONTSIZE, InpMiniHUD_FontSize);
      if(!InpMiniHUDFollowPrice)
      {
         ObjectSetInteger(cid, MINI_HUD, OBJPROP_CORNER, g_miniCorner);
         ObjectSetInteger(cid, MINI_HUD, OBJPROP_XDISTANCE, g_miniX);
         ObjectSetInteger(cid, MINI_HUD, OBJPROP_YDISTANCE, g_miniY);
      }
      else
      {
         ObjectSetInteger(cid, MINI_HUD, OBJPROP_ANCHOR, ANCHOR_LEFT_LOWER);
      }
   }
}
...
void UpdateMiniHUD()
{
   if(!InpMiniHUD)
   {
      if(ObjectFind(ChartID(), MINI_HUD) != -1) ObjectDelete(ChartID(), MINI_HUD);
      if(InpMiniHUD_UseComment) Comment("");
      return;
   }
   EnsureMiniHUDObject();

   Regime reg = DetectRegime();
   char rc = (reg==REG_TREND?'T':(reg==REG_RANGE?'R':(reg==REG_SHOCK?'S':'-')));
   double k = RiskScalerK();
   bool gate = EquityFilterAllows() && !g_paused && (g_cooldownUntil==0 || TimeCurrent()>=g_cooldownUntil);
   ...
   string txt = StringFormat("ORION v7 | %c | k=%.2f | Gate:%s | Pos:%d | Risk:%.1f%%",
                              rc, k, (gate?"ON":"OFF"), posCount, openRisk);

   if(InpMiniHUDFollowPrice)
   {
      SymbolInfoTick(_Symbol, g_tick);
      datetime t0 = iTime(_Symbol, PERIOD_CURRENT, 0);
      double   y  = g_tick.bid + PointsToPrice(g_miniOffsetPts);
      if(ObjectFind(ChartID(), MINI_HUD) != -1)
      {
         ObjectSetInteger(ChartID(), MINI_HUD, OBJPROP_TIME, t0);
         ObjectSetDouble (ChartID(), MINI_HUD, OBJPROP_PRICE, y);
         ObjectSetString (ChartID(), MINI_HUD, OBJPROP_TEXT, txt);
      }
   }
   else
   {
      ...
      {
         ObjectSetString (ChartID(), MINI_HUD, OBJPROP_TEXT, txt);
         ObjectSetInteger(ChartID(), MINI_HUD, OBJPROP_CORNER, g_miniCorner);
         ObjectSetInteger(ChartID(), MINI_HUD, OBJPROP_XDISTANCE, g_miniX);
         ObjectSetInteger(ChartID(), MINI_HUD, OBJPROP_YDISTANCE, g_miniY);
      }
   }

   if(InpMiniHUD_UseComment)
      Comment(txt);
}

//============================= DASHBOARD ==============================//
void LayoutPanelChildren()
{
   long cid = ChartID();

   // Handle position (top-left corner inside the panel)
   if(ObjectFind(cid, PANEL_HANDLE) != -1)
   {
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_CORNER, g_panelCorner);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_XDISTANCE, g_panelX+6);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_YDISTANCE, g_panelY+6);
   }

   // Panel text
   if(ObjectFind(cid, PANEL_TEXT) != -1)
   {
      ObjectSetInteger(cid, PANEL_TEXT, OBJPROP_CORNER, g_panelCorner);
      ObjectSetInteger(cid, PANEL_TEXT, OBJPROP_XDISTANCE, g_panelX+10);
      ObjectSetInteger(cid, PANEL_TEXT, OBJPROP_YDISTANCE, g_panelY+10);
   }
   // Buttons
   if(InpShowButtons)
   {
      if(ObjectFind(cid, BTN_PAUSE) != -1)
      {
         ObjectSetInteger(cid, BTN_PAUSE, OBJPROP_CORNER, g_panelCorner);
         ObjectSetInteger(cid, BTN_PAUSE, OBJPROP_XDISTANCE, g_panelX+10);
         ObjectSetInteger(cid, BTN_PAUSE, OBJPROP_YDISTANCE, g_panelY+InpPanelHeight-50);
      }
      if(ObjectFind(cid, BTN_FLAT) != -1)
      {
         ObjectSetInteger(cid, BTN_FLAT, OBJPROP_CORNER, g_panelCorner);
         ObjectSetInteger(cid, BTN_FLAT, OBJPROP_XDISTANCE, g_panelX+110);
         ObjectSetInteger(cid, BTN_FLAT, OBJPROP_YDISTANCE, g_panelY+InpPanelHeight-50);
      }
   }
}
...
void CreatePanel()
{
   ...
   if(ObjectFind(cid, PANEL_BG) == -1)
   {
      ObjectCreate(cid, PANEL_BG, OBJ_RECTANGLE_LABEL, 0, 0, 0);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_CORNER, g_panelCorner);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_XDISTANCE, g_panelX);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_YDISTANCE, g_panelY);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_XSIZE, InpPanelWidth);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_YSIZE, InpPanelHeight);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_COLOR, clrBlack);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_BGCOLOR, clrDimGray);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_BACK, !InpPanelFront);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_SELECTABLE, InpPanelDraggable);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_SELECTED, false);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_HIDDEN, false);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_ZORDER, 0);
   }
   else
   {
      // Refresh geometry if inputs changed
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_XSIZE, InpPanelWidth);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_YSIZE, InpPanelHeight);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_BACK, !InpPanelFront);
      ObjectSetInteger(cid, PANEL_BG, OBJPROP_SELECTABLE, InpPanelDraggable);
   ...
   if(ObjectFind(cid, PANEL_TEXT) == -1)
   {
      ...
      ObjectSetInteger(cid, PANEL_TEXT, OBJPROP_HIDDEN, false);
   ...
   if(ObjectFind(cid, PANEL_HANDLE) == -1)
   {
      ObjectCreate(cid, PANEL_HANDLE, OBJ_LABEL, 0, 0, 0);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_COLOR, clrGold);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_SELECTABLE, true);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_SELECTED, false);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_HIDDEN, false);
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_ZORDER, 3);
      ObjectSetString (cid, PANEL_HANDLE, OBJPROP_FONT, "Consolas");
      ObjectSetInteger(cid, PANEL_HANDLE, OBJPROP_FONTSIZE, 12);
      ObjectSetString (cid, PANEL_HANDLE, OBJPROP_TEXT, "≡");
   }
...
void UpdateDashboard()
{
   ...
   if(InpShowButtons && ObjectFind(ChartID(), BTN_PAUSE) != -1)
...
int OnInit()
{
   ...
   // Initialize runtime UI positions
   g_panelCorner = InpPanelCorner; g_panelX = InpPanelX; g_panelY = InpPanelY;
   g_miniCorner  = InpMiniHUD_Corner; g_miniX = InpMiniHUD_X; g_miniY = InpMiniHUD_Y;
   g_miniOffsetPts = InpMiniHUDOffsetPts;
...
void OnDeinit(const int reason)
{
   ...
   // Clean panel objects
   long cid = ChartID();
   if(ObjectFind(cid, PANEL_BG)     != -1) ObjectDelete(cid, PANEL_BG);
   if(ObjectFind(cid, PANEL_TEXT)   != -1) ObjectDelete(cid, PANEL_TEXT);
   if(ObjectFind(cid, BTN_PAUSE)    != -1) ObjectDelete(cid, BTN_PAUSE);
   if(ObjectFind(cid, BTN_FLAT)     != -1) ObjectDelete(cid, BTN_FLAT);
   if(ObjectFind(cid, PANEL_HANDLE) != -1) ObjectDelete(cid, PANEL_HANDLE);
   if(ObjectFind(cid, HL_P1)        != -1) ObjectDelete(cid, HL_P1);
   if(ObjectFind(cid, HL_P2)        != -1) ObjectDelete(cid, HL_P2);
   if(ObjectFind(cid, MINI_HUD)     != -1) ObjectDelete(cid, MINI_HUD);
   if(InpMiniHUD_UseComment) Comment("");
}
...
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
{
   ...
   {
      if(sparam==BTN_PAUSE){ g_paused = !g_paused; UpdateDashboard(); UpdateMiniHUD(); }
      if(sparam==BTN_FLAT ){ CloseAllEA(); UpdateDashboard(); UpdateMiniHUD(); }
   }
   else if(id==CHARTEVENT_OBJECT_DRAG)
   {
      // --- Smart drag with handle (snap panel to nearest corner) ---
      if(sparam==PANEL_HANDLE && ObjectFind(ChartID(), PANEL_HANDLE)!=-1)
      {
         long cid=ChartID();

         // Current handle position relative to its corner
         int hcorner = (int)ObjectGetInteger(cid, PANEL_HANDLE, OBJPROP_CORNER);
         int hx = (int)ObjectGetInteger(cid, PANEL_HANDLE, OBJPROP_XDISTANCE);
         int hy = (int)ObjectGetInteger(cid, PANEL_HANDLE, OBJPROP_YDISTANCE);

         // Chart size
         int cw = (int)ChartGetInteger(cid, CHART_WIDTH_IN_PIXELS);
         int ch = (int)ChartGetInteger(cid, CHART_HEIGHT_IN_PIXELS);

         // Absolute pixel coords of the handle glyph
         int ax=0, ay=0;
         if(hcorner==CORNER_LEFT_UPPER)   { ax = hx;        ay = hy; }
         if(hcorner==CORNER_RIGHT_UPPER)  { ax = cw - hx;   ay = hy; }
         if(hcorner==CORNER_LEFT_LOWER)   { ax = hx;        ay = ch - hy; }
         if(hcorner==CORNER_RIGHT_LOWER)  { ax = cw - hx;   ay = ch - hy; }

         // Choose nearest corner
         int distLU = ax + ay;
         int distRU = (cw-ax) + ay;
         int distLL = ax + (ch-ay);
         int distRL = (cw-ax) + (ch-ay);

         int newCorner = g_panelCorner;
         int best = distLU; newCorner = CORNER_LEFT_UPPER;
         if(distRU<best){ best=distRU; newCorner=CORNER_RIGHT_UPPER; }
         if(distLL<best){ best=distLL; newCorner=CORNER_LEFT_LOWER; }
         if(distRL<best){ best=distRL; newCorner=CORNER_RIGHT_LOWER; }

         // Distances from the selected corner for panel origin (keep handle near 6,6 inside panel)
         int px=0, py=0;
         if(newCorner==CORNER_LEFT_UPPER)   { px = ax-6;       py = ay-6; }
         if(newCorner==CORNER_RIGHT_UPPER)  { px = (cw-ax)-6;  py = ay-6; }
         if(newCorner==CORNER_LEFT_LOWER)   { px = ax-6;       py = (ch-ay)-6; }
         if(newCorner==CORNER_RIGHT_LOWER)  { px = (cw-ax)-6;  py = (ch-ay)-6; }

         // Clamp to viewport
         if(px<0) px=0; if(py<0) py=0;
         if(px>cw-10) px=cw-10; if(py>ch-10) py=ch-10;

         g_panelCorner=newCorner; g_panelX=px; g_panelY=py;

         // Apply to background & children
         if(ObjectFind(cid, PANEL_BG)!=-1)
         {
            ObjectSetInteger(cid, PANEL_BG, OBJPROP_CORNER, g_panelCorner);
            ObjectSetInteger(cid, PANEL_BG, OBJPROP_XDISTANCE, g_panelX);
      ...
      // --- Dragging the background directly (if selectable) ---
      if(sparam==PANEL_BG && ObjectFind(ChartID(), PANEL_BG)!=-1)
      {
         g_panelCorner = (int)ObjectGetInteger(ChartID(), PANEL_BG, OBJPROP_CORNER);
         g_panelX      = (int)ObjectGetInteger(ChartID(), PANEL_BG, OBJPROP_XDISTANCE);
         g_panelY      = (int)ObjectGetInteger(ChartID(), PANEL_BG, OBJPROP_YDISTANCE);
         LayoutPanelChildren();
      }

      // --- Mini HUD drag ---
      if(sparam==MINI_HUD && ObjectFind(ChartID(), MINI_HUD)!=-1)
      {
         int t = (int)ObjectGetInteger(ChartID(), MINI_HUD, OBJPROP_TYPE);
         if(t==OBJ_LABEL) // screen-anchored
         {
            g_miniCorner = (int)ObjectGetInteger(ChartID(), MINI_HUD, OBJPROP_CORNER);
            g_miniX      = (int)ObjectGetInteger(ChartID(), MINI_HUD, OBJPROP_XDISTANCE);
            g_miniY      = (int)ObjectGetInteger(ChartID(), MINI_HUD, OBJPROP_YDISTANCE);
         }
         else if(t==OBJ_TEXT && InpMiniHUDFollowPrice) // price-follow mode: adjust vertical offset
         {
            double py = ObjectGetDouble(ChartID(), MINI_HUD, OBJPROP_PRICE);
            SymbolInfoTick(_Symbol, g_tick);
            g_miniOffsetPts = PriceToPoints(py - g_tick.bid);
         }
      }
   }
}
