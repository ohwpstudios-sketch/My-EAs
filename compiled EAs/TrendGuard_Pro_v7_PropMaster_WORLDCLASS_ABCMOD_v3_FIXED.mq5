...
input string  TelemetryFileName           = "TG7_Telemetry.csv"; // saved in common files folder
...
//------------------- Telemetry -------------------------------------
void TelemetryWriteHeader(int handle){
   // timestamp,symbol,event,reason,risk_pct,spread_pts,rr,equity,balance,retcode
   FileWrite(handle, "timestamp","symbol","event","reason","risk_pct","spread_pts","rr","equity","balance","retcode");
}
void TelemetryLog(const string event, const string sym, const string reason, double risk_pct, double spread_pts, double rr, long retcode=0){
   if(!EnableTelemetry) return;
   int flags = FILE_WRITE|FILE_READ|FILE_CSV|FILE_ANSI|FILE_COMMON;
   int handle = FileOpen(TelemetryFileName, flags);
   if(handle==INVALID_HANDLE) return;
   if(FileSize(handle)==0) TelemetryWriteHeader(handle);
   FileSeek(handle, 0, SEEK_END);
   string ts = TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS);
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   double bal= AccountInfoDouble(ACCOUNT_BALANCE);
   FileWrite(handle, ts, sym, event, reason, DoubleToString(risk_pct,2), DoubleToString(spread_pts,1), DoubleToString(rr,2), DoubleToString(eq,2), DoubleToString(bal,2), (int)retcode);
   FileClose(handle);
}
double SpreadPoints(const string sym){ double p=SymbolInfoDouble(sym,SYMBOL_POINT); if(p<=0) return 0.0; return (SymbolInfoDouble(sym,SYMBOL_ASK)-SymbolInfoDouble(sym,SYMBOL_BID))/p; }
...
void UpdateDashboard(){
   ...
   double budgetUsed = g_dayRiskBudgetSpentPct;
   double budgetMax  = DailyRiskBudget_Percent;
   double openRisk = 0.0;
   // Compute total open risk for this EA
   for(int i=0;i<PositionsTotal();i++){ ulong tk=PositionGetTicket(i); if(!PositionSelectByTicket(tk)) continue;
      if((long)PositionGetInteger(POSITION_MAGIC)!=MagicNumber) continue;
      string s=PositionGetString(POSITION_SYMBOL);
      openRisk += ComputePositionRiskPct(s, MagicNumber);
   }
   string regimeStr="-";
   Regime r = DetectRegime();
   if(r==REGIME_TREND) regimeStr="TREND"; else if(r==REGIME_RANGE) regimeStr="RANGE"; else regimeStr="CHAOS";
   double spr = SpreadPoints(_Symbol);
   string lines;
   lines =  "TG7 Dashboard  |  " + _Symbol + "  TF=" + EnumToString((ENUM_TIMEFRAMES)_Period) + "\n";
   lines += "Equity: " + DoubleToString(eq,2) + "   Balance: " + DoubleToString(bal,2) + "\n";
   lines += "Day P/L: " + DoubleToString(dayPL,2) + "%    Budget Used: " + DoubleToString(budgetUsed,2) + "% / " + DoubleToString(budgetMax,2) + "%\n";
   lines += "Open Risk: " + DoubleToString(openRisk,2) + "%    Spread: " + DoubleToString(spr,1) + " pts    Regime: " + regimeStr + "\n";
   if(StringLen(g_lastBlockReason)>0) lines += "Last Block: " + g_lastBlockReason + "\n";
   Comment(lines);
}
...
double ComputePositionRiskPct(const string sym, long magic){
   ...
   if(sl<=0) return 0.0; // no SL (shouldn't happen, we set it)
...
double GetATR_S(string sym, int period, ENUM_TIMEFRAMES tf, int shift){
   if(!SeriesReady(sym, tf, MinBarsForIndicators)) return 0.0;
...
bool TryPlaceMarket(const string sym, int dir, double lots, double sl, double tp, const string cmt){
   ...
   // Fallback on typical rejections
   ...
   if(FallbackSendWithoutSLTP && (rc==TRADE_RETCODE_INVALID_STOPS || rc==TRADE_RETCODE_REQUOTE || rc==TRADE_RETCODE_PRICE_OFF || rc==TRADE_RETCODE_MARKET_CLOSED || rc==TRADE_RETCODE_INVALID_PRICE)){
      ...
      if(dir>0) ok = Trade.Buy(lots, sym, 0.0, 0.0, 0.0, cmt);
      else      ok = Trade.Sell(lots, sym, 0.0, 0.0, 0.0, cmt);
      if(!ok) return false;
...
bool PlaceTrade(int dir, double stop_pips, double rr_tp){
   ...
   double spreadPts = SpreadPoints(_Symbol);
   if(spreadPts > MaxSpreadPoints){
      g_lastBlockReason="SpreadTooHigh";
      TelemetryLog("BLOCK", _Symbol, g_lastBlockReason, 0.0, spreadPts, rr_tp);
      if(VerboseLog) Print("TG7: Spread too high (", DoubleToString(spreadPts,1), ">", MaxSpreadPoints, "). Skip.");
      return false;
   }

   // Effective risk = base * sessions * DoW
   double sessMult=1.0; if(!SessionOkayAndSizeMult(sessMult)){ g_lastBlockReason="SessionFilter"; TelemetryLog("BLOCK", _Symbol, g_lastBlockReason, 0.0, SpreadPoints(_Symbol), rr_tp); return false; }
   if(IsBlockedByNewsStub()){ g_lastBlockReason="NewsBlock"; TelemetryLog("BLOCK", _Symbol, g_lastBlockReason, 0.0, SpreadPoints(_Symbol), rr_tp); return false; }
   if(!CheckDailyAndTotalDD()){ g_lastBlockReason="GuardrailDD"; TelemetryLog("BLOCK", _Symbol, g_lastBlockReason, 0.0, SpreadPoints(_Symbol), rr_tp); return false; }

   double dowMult = GetDoWSizeMult();
   double effRisk = BaseRisk_Percent * sessMult * dowMult;

   // Portfolio gates (apply even for single-chart mode)
   string reason;
   if(!BudgetAndExposureAllow(_Symbol, effRisk, reason)){ g_lastBlockReason=reason; TelemetryLog("BLOCK", _Symbol, g_lastBlockReason, effRisk, SpreadPoints(_Symbol), rr_tp); if(VerboseLog) Print("TG7: Portfolio gate blocked: ", reason); return false; }
   ...
   // Normalize lots & ensure SL distance
   sl = EnsureStopDistance(_Symbol, price, sl, (dir>0));

   string cmt = "TG7 RPTS=" + IntegerToString((int)MathRound(stop_dist_points/point));
   bool ok = TryPlaceMarket(_Symbol, dir, lots, sl, tp, cmt);
   if(!ok){
      if(VerboseLog) Print("TG7: Order send failed. Retcode=", Trade.ResultRetcode());
      return false;
   }
   TelemetryLog("ENTRY", _Symbol, "", effRisk, spreadPts, rr_tp, Trade.ResultRetcode());
...
bool PlaceTradeSym(const string sym, int dir, double stop_pips, double rr_tp){
   ...
   double minStopPoints = (double)stopsLvl;
   if(minStopPoints>0 && stop_dist_points < minStopPoints*point) stop_dist_points = minStopPoints*point;
   double spreadPts = SpreadPoints(sym);
   if(spreadPts > MaxSpreadPoints){ g_lastBlockReason="SpreadTooHigh"; TelemetryLog("BLOCK", sym, g_lastBlockReason, 0.0, spreadPts, rr_tp); if(VerboseLog) Print("TG7: Spread too high @", sym, " sp=", spreadPts); return false; }

   double sessMult=1.0; if(!SessionOkayAndSizeMult(sessMult)){ g_lastBlockReason="SessionFilter"; TelemetryLog("BLOCK", sym, g_lastBlockReason, 0.0, SpreadPoints(sym), rr_tp); return false; }
   if(IsBlockedByNewsStub()){ g_lastBlockReason="NewsBlock"; TelemetryLog("BLOCK", sym, g_lastBlockReason, 0.0, SpreadPoints(sym), rr_tp); return false; }
   if(!CheckDailyAndTotalDD()){ g_lastBlockReason="GuardrailDD"; TelemetryLog("BLOCK", sym, g_lastBlockReason, 0.0, SpreadPoints(sym), rr_tp); return false; }

   double dowMult = GetDoWSizeMult();
   double effRisk = BaseRisk_Percent * sessMult * dowMult;

   string reason;
   if(!BudgetAndExposureAllow(sym, effRisk, reason)){ g_lastBlockReason=reason; TelemetryLog("BLOCK", sym, g_lastBlockReason, effRisk, SpreadPoints(sym), rr_tp); if(VerboseLog) Print("TG7: Portfolio gate blocked: ", reason); return false; }
   ...
   BudgetConsume(effRisk);
   TelemetryLog("ENTRY", sym, "", effRisk, spreadPts, rr_tp, Trade.ResultRetcode());
...
int OnInit(){
   ...
   // Prime history for chart symbol
...
void OnTick(){
   if(EnableMultiSymbolScan) { UpdateDashboard(); return; } // scanning handled by OnTimer
   // Only run once per bar for signal logic
   datetime cur_bar = iTime(_Symbol, PERIOD_CURRENT, 0);
   if(cur_bar == last_bar_time){ UpdateDashboard(); return; }
   last_bar_time = cur_bar;

   if(!CheckDailyAndTotalDD()){ UpdateDashboard(); return; }
...
// Portfolio manager (partial TP / BE / Hybrid trailing)
...
// Timer scan for portfolio mode
void OnTimer(){
   if(!EnableMultiSymbolScan) return;
   // reset daily anchor/budget when day rolls
   datetime curDay = iTime(_Symbol,PERIOD_D1,0);
