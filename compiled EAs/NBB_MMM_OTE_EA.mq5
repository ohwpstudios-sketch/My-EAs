...
int OnInit()
{
   ...
   if(InpFibEntryMin < 0.30 || InpFibEntryMin > 0.79) { Print("FibEntryMin adjusted to 0.50"); InpFibEntryMin = 0.50; }
   if(InpFibEntryMax <= InpFibEntryMin || InpFibEntryMax > 0.90) { Print("FibEntryMax adjusted to 0.79"); InpFibEntryMax = 0.79; }
   if(InpSecondaryFib <= InpFibEntryMin || InpSecondaryFib >= InpFibEntryMax) InpSecondaryFib = 0.705;
...
void BuildFibLegPrices(FibLeg &leg)
{
   ...
   if(leg.isShort)
   {
      ...
      double pMin = leg.B + range*InpFibEntryMin;
      double pMax = leg.B + range*InpFibEntryMax;
      leg.entryMin = pMin;
      leg.entryMax = pMax;
      leg.fib0 = leg.B;
   ...
   else
   {
      ...
      double pMin = leg.A + range*InpFibEntryMin;
      double pMax = leg.A + range*InpFibEntryMax;
      leg.entryMin = pMin;
      leg.entryMax = pMax;
      leg.fib0 = leg.B;
...
bool OpenOTEOrders(const FibLeg &leg)
{
   ...
   if(leg.isShort)
   {
      ...
      if(InpUseDualLimits) entry2 = leg.A - (leg.A - leg.B)*InpSecondaryFib;
   }
   else
   {
      entry1 = leg.A + (leg.B - leg.A)*0.62;
      if(InpUseDualLimits) entry2 = leg.A + (leg.B - leg.A)*InpSecondaryFib;
   }
   ...
   if(entry1>0.0)
   {
      ...
      {
         bool ok;
         if(leg.isShort) ok = Trade.SellLimit(lot, entry1, _Symbol, sl, tp, InpMaxSlippagePoints);
         else            ok = Trade.BuyLimit (lot, entry1, _Symbol, sl, tp, InpMaxSlippagePoints);
   ...
   if(entry2>0.0)
   {
      ...
      {
         bool ok;
         if(leg.isShort) ok = Trade.SellLimit(lot, entry2, _Symbol, sl, tp, InpMaxSlippagePoints);
         else            ok = Trade.BuyLimit (lot, entry2, _Symbol, sl, tp, InpMaxSlippagePoints);
...
void ManageOpenPositions()
{
   ...
   if(g_scaleIns < InpMaxScaleIns && g_leg.valid)
   {
      ...
      if(have && safe)
      {
         ...
         double lot = CalculateLotSize(stopPts);
         bool ok=false;
         if(g_leg.isShort) ok = Trade.SellLimit(lot, mid, _Symbol, stop, tp, InpMaxSlippagePoints);
         else              ok = Trade.BuyLimit (lot, mid, _Symbol, stop, tp, InpMaxSlippagePoints);
