...
//============================= INPUTS ===============================
input long    MagicNumber                       = 990002025;

// --- Symbols & engine
input string  SymbolsCSV                        = "EURUSD,GBPUSD,XAUUSD,NAS100,BTCUSD";
input string PreferredSuffix = "m";      // Prefer broker symbols ending with this suffix (e.g., Exness uses "m")
...
// [FIX] removed stray 'return out;' from old helper
// [FIX] removed stray '}' from old helper
...
void GetAliasesForBase(string base_in, string &aliases[])
  {
   ...
   if(base=="GER40" || base=="DE40" || base=="DAX40" || base=="GER30" || base=="DE30" || base=="DAX")
     {
      string a[]={"GER40","DE40","DAX40","GER30","DE30","DAX","GER40.cash","DE40.cash","GER40m","DE40m", "GER30m","DE30m"};
      for(int i=0;i<ArraySize(a);i++) PushUnique(aliases, a[i]);
     }
   ...
   // ------ More Indices ------
   if(base=="JP225" || base=="JPN225" || base=="NIKKEI" || base=="N225")
     { string a[]={"JP225","JPN225","NIKKEI","N225","JP225.cash","JP225m"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="HK50" || base=="HKG50" || base=="HSI" || base=="HKHI")
     { string a[]={"HK50","HKG50","HSI","HKHI","HK50.cash","HK50m"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="FRA40" || base=="FR40" || base=="CAC40" || base=="CAC")
     { string a[]={"FRA40","FR40","CAC40","CAC","FRA40.cash","FRA40m"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="ESP35" || base=="ES35" || base=="IBEX35" || base=="IBEX")
     { string a[]={"ESP35","ES35","IBEX35","IBEX","ESP35.cash","ESP35m"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="AUS200" || base=="AU200" || base=="ASX200" || base=="AUS")
     { string a[]={"AUS200","AU200","ASX200","AUS","AUS200.cash","AUS200m"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }

   // ------ Additional Metals ------
   if(base=="XPTUSD" || base=="PLATINUM" || base=="XPT")
     { string a[]={"XPTUSD","PLATINUM","XPTUSDm","XPTUSD.cash"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="XPDUSD" || base=="PALLADIUM" || base=="XPD")
     { string a[]={"XPDUSD","PALLADIUM","XPDUSDm","XPDUSD.cash"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }

   // ------ Gas ------
   if(base=="NATGAS" || base=="XNGUSD" || base=="NGAS")
     { string a[]={"NATGAS","XNGUSD","NGAS","NATGAS.cash","XNGUSDm"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }

   // ------ More Crypto (CFD labels) ------
   if(base=="LTCUSD" || base=="LTCUSDT" || base=="LTC")
     { string a[]={"LTCUSD","LTCUSDT","LTCUSDm","LTCUSD.r","LTCUSD.cash"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
   if(base=="XRPUSD" || base=="XRPUSDT" || base=="XRP")
     { string a[]={"XRPUSD","XRPUSDT","XRPUSDm","XRPUSD.r","XRPUSD.cash"};
       for(int i=0;i<ArraySize(a);i++) PushUnique(aliases,a[i]); }
  }
  }
...
string ResolveSymbol(const string base_in)
  {
   ...
   // Preferred-suffix exact match (e.g., alias+"m" for Exness)
   if(StringLen(PreferredSuffix)>0){
      int total_ps = (int)SymbolsTotal(false);
      for(int i=0;i<total_ps;i++){
         string sym_ps = SymbolName(i,false);
         for(int k=0;k<ArraySize(aliases);k++){
            string expect = aliases[k] + PreferredSuffix;
            if(StringCompare(sym_ps, expect, true)==0){
               SymbolSelect(sym_ps,true);
               Print("[Resolver] ", base_in, " -> ", sym_ps, " (exact+suffix)");
               return sym_ps;
            }
         }
      }
   ...
   // Second pass (biased): substring/prefix/suffix with PreferredSuffix priority
   string best_fuzzy = "";
   for(int i=0;i<total_all;i++)
     {
      string sym = SymbolName(i,false);
      string ns = Normalize(sym);
      // direct containment against base and first alias
      bool hit = (StringFind(ns, Normalize(aliases[0]))>=0 || StringFind(ns, norm_base)>=0);
      if(!hit){
         // match any alias by containment/prefix/suffix
         for(int k=0;k<ArraySize(aliases);k++)
           {
            string na = Normalize(aliases[k]);
            if(StringFind(ns, na)>=0 || EndsWith(ns, na) || StartsWith(ns, na)){ hit=true; break; }
           }
      }
      if(hit){
         if(StringLen(PreferredSuffix)>0 && EndsWith(sym, PreferredSuffix)){
            SymbolSelect(sym,true);
            Print("[Resolver] ", base_in, " -> ", sym, " (fuzzy+suffix)");
            return sym;
         }
         if(best_fuzzy=="") best_fuzzy = sym;
      }
     }
   if(best_fuzzy!=""){
      SymbolSelect(best_fuzzy,true);
      Print("[Resolver] ", base_in, " -> ", best_fuzzy, " (fuzzy)");
      return best_fuzzy;
   }

      // Third pass (biased): prioritize visible symbols with PreferredSuffix priority
      ...
      if(StringFind(ns, norm_base)>=0)
        {
         if(StringLen(PreferredSuffix)>0 && EndsWith(sym, PreferredSuffix)){
            SymbolSelect(sym,true);
            Print("[Resolver] ", base_in, " -> ", sym, " (visible-fallback+suffix)");
            return sym;
         }
         if(best_vis=="") best_vis = sym;
        }
     }
   if(best_vis!=""){
      SymbolSelect(best_vis,true);
      Print("[Resolver] ", base_in, " -> ", best_vis, " (visible-fallback)");
      return best_vis;
   }
   // [FIX] removed extra brace (cleanup)

   // No match
   Print("[Resolver] Could not resolve base symbol: ", base_in);
   return "";
}
