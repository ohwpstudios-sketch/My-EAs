//+------------------------------------------------------------------+
//|                                        ORION_AllWeather_Engine_v6d|
//|                                   Author: GH Wealth Makers Elite   |
//|                                         (c) 2025, GH Wealth Makers |
//+------------------------------------------------------------------+
//| A multi-regime "all-weather" trading engine for any liquid symbol |
//| Adds a movable dashboard panel, a movable Mini HUD (price-follow  |
//| or screen-anchored), adaptive params, circuit breakers, equity MA |
//| gate, partial TP layers, and ATR trailing with no-op suppression. |
//+------------------------------------------------------------------+
...
input int     InpPanelHeight           = 180;        // Panel height
...
void EnsureMiniHUDObject()
{
   ...
   if(ObjectFind(cid, MINI_HUD) == -1)
   {
      ...
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_ZORDER, 2);
...
void UpdateMiniHUD()
{
   ...
   string txt = StringFormat("ORION v6 | %c | k=%.2f | Gate:%s | Pos:%d | Risk:%.1f%%",
...
{
   long cid = ChartID();
   // Respect background location (g_panelCorner, g_panelX, g_panelY)
...
void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam)
{
   ...
   else if(id==CHARTEVENT_OBJECT_DRAG)
   {
      ...
      if(sparam==MINI_HUD && ObjectFind(ChartID(), MINI_HUD)!=-1)
      {
         ...
         else if(t==OBJ_TEXT && InpMiniHUDFollowPrice) // price-follow mode: adjust offset by drag
         {
