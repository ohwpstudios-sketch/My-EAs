...
//============================= INPUTS ===============================
// --- Identification
input long    MagicNumber                 = 7700770890;
...
bool TryPlaceMarket(const string sym, int dir, double lots, double sl, double tp){
   ...
   if(dir>0){
      if(wantSLTP) ok = Trade.Buy(lots, sym, 0.0, sl, tp, "TG7");
      else         ok = Trade.Buy(lots, sym, 0.0, 0.0, 0.0, "TG7");
   }else{
      if(wantSLTP) ok = Trade.Sell(lots, sym, 0.0, sl, tp, "TG7");
