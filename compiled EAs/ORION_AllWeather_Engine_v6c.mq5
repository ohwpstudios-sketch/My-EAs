...
//| With vol targeting, spread/slippage guards, sessions, partial-Tp, |
//| dashboard, circuit breaker, equity filter, adaptive params,       |
//| buttons, and a compact Mini HUD that follows price.               |
//+------------------------------------------------------------------+
...
// Mini HUD (price-following)
input bool    InpMiniHUD               = true;       // Show compact mini HUD
input int     InpMiniHUDOffsetPts      = 80;         // Vertical offset in points above price
   ...
   for(int i=0;i<PositionsTotal();i++)
   {
      ...
      if(InpUsePartialTP && !g_memos[idx].partial1Done)
      {
         ...
         if(hit1)
         {
            ...
            {
               // Mark to avoid re-check loops on tiny positions
               g_memos[idx].partial1Done=true;
            }
         }
      }
...
void CreateMiniHUD()
{
   ...
   {
      ObjectCreate(cid, MINI_HUD, OBJ_TEXT, 0, 0, 0);
      ...
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_SELECTABLE, false);
      ObjectSetInteger(cid, MINI_HUD, OBJPROP_HIDDEN, true);
...
void UpdateMiniHUD()
{
   ...
   double   y  = g_tick.bid + PointsToPrice(InpMiniHUDOffsetPts);
