...
//|        2025-09 - Final Warning-Free Build (MT5)                  |
//+------------------------------------------------------------------+
...
//============================= INPUTS ===============================
input long    MagicNumber                       = 88058805;
...
// --- Risk & guardrails
input double  BaseRisk_Percent                  = 0.30;
input double  MaxDailyLoss_Percent              = 4.5;
input double  MaxTotalDrawdown_Percent          = 9.0;
input int     MaxConsecutiveLosses              = 6;
input int     CooldownMinutesAfterLoss          = 30;
input int     MaxOpenPositionsPerSymbol         = 1;
input int     MaxOpenPositionsTotal             = 5;
input double  MaxPortfolioRisk_Percent          = 1.20;
...
input int     ADX_TrendThreshold                 = 25;
input int     ADX_RangeThreshold                 = 20;
...
// ✅ Warning-free: all-int arithmetic
...
bool SessionOkayAndSizeMult(double &mult_out){
   ...
   if(DebugMode){
      string dbg = StringFormat("TG8 Debug Session: tok=%d lon=%d ny=%d mult=%.2f",
...
bool PlaceTrade(const string sym,int dir,double stop_pips,double rr_tp){
   ...
   if(activeRisk + effRisk > MaxPortfolioRisk_Percent){
      string msg=StringFormat("TG8: Portfolio risk cap reached. Active=%.2f%% + New=%.2f%% > Max=%.2f%%",
   ...
   if(VerboseLog){
      string lg=StringFormat("TG8: %s %s risk=%.2f%% stop_pips=%.1f rr=%.2f",
...
int OnInit(){
   ...
   string initMsg=StringFormat("TG8 Init: TF=%s Symbols=%s", EnumToString(WorkingTF), SymbolsCSV);
   Print(initMsg);
...
void DrawDashboard(){
   ...
   string line = StringFormat(
      "Equity: %.2f | DD Day: %.2f%%/%.1f%% | DD Total: %.2f%%/%.1f%% | RiskMode: %s",
